sap.ui.define([
    ...
    return BaseController.extend("opensap.project1.controller.List", {
        ...
        onInit : function () {
            ...
            this.getRouter().getRoute("addEmployee").attachPatternMatched(this._onMasterMatched, this);
        ...
        onOpenViewSettings: function (oEvent) {
            ...
            if (!this.byId("viewSettingsDialog")) {
                Fragment.load({
                    ...
                    name: "opensap.project1.view.ViewSettingsDialog",
                    controller: this
        ...
        _applyListFilter : function(){
            var filterItems= this.byId("viewSettingsDialog").getFilterItems();
            var aFilters=[];
            var oBinding= this.byId("list").getBinding("items");

            this._resetFilterCounter();

            filterItems.forEach( oItem => {
                var sKey=oItem.getKey();
                var sValue=oItem.getCustomControl().getValue();
                if(sValue != ""){
                    aFilters.push(new Filter(sKey , FilterOperator.Contains , sValue));
                    oItem.setProperty("filterCount" , 1);
                }
            });
            ...
            filterItems.forEach(oItem => {
                oItem.getCustomControl().setValue("");
            });
        ...
        _resetFilterCounter : function(){
            var filterItems= this.byId("viewSettingsDialog").getFilterItems();
            filterItems.forEach( oItem => {
                oItem.setProperty("filterCount" , 0);
            });
        },
        /**
        ...
        _showDetail: function (oItem) {
            ...
            this.getRouter().navTo("object", {
                objectId : oItem.getBindingContext().getProperty("IdHead")
            }, bReplace);
        },
        ...
        onDeleteEmployee : function(){
            ...
            if(!oList.getSelectedItem()){
              MessageToast.show(oResourceBundle.getText("deleteNoSelect"));
            }else{
                MessageBox.confirm(oResourceBundle.getText("confirmDeleteMessage") , {
                    title: oResourceBundle.getText("messageBoxDeleteEmployeeTitle"),
                    onClose: function(sAction){
                        if(sAction === "OK"){
                            this.getModel().remove(oList.getSelectedItem().getBindingContext().getPath() , {
                                success: function(){
                                    MessageToast.show(oResourceBundle.getText("deleteEmployeeSuccess"));
                                    that.getRouter().navTo("list");
                                    oList.removeSelections(true);
                                },
                                error: function(){
                                    MessageToast.show("Error while deleting employee!");
                                }
                            });
                        }else{
                            that.getRouter().navTo("list");
